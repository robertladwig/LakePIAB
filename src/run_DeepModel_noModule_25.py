...
from processBased_lakeModel_functions import get_hypsography, provide_meteorology, initial_profile, run_thermalmodel, run_thermalmodel, heating_module, diffusion_module, mixing_module, convection_module, ice_module, run_thermalmodel_hybrid, run_thermalmodel_hybrid_v2, run_thermalmodel_deep, run_thermalmodel_deep_noModule
...
res = run_thermalmodel_deep_noModule(
    u = deepcopy(u_ini),
    startTime = startTime, 
...
df.to_csv('../verification/dlnoM_temp_25.csv', index=None)
...
df.to_csv('../verification/dlnoM_icesnow_25.csv', index=None)
...
df = pd.concat([df1, df2], axis = 1)
df.to_csv('../verification/dlnoM_meteorology_25.csv', index=None)