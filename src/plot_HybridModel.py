...
n_time = int(number_days * training_frac)

times_validation = times[n_time:]
temp_validation = temp[0:,n_time:]
...
sns.heatmap(temp_validation, cmap=plt.cm.get_cmap('Spectral_r'),  xticklabels=1000, yticklabels=2, vmin = 0, vmax = 35)
ax.set_ylabel("Depth", fontsize=15)
ax.set_xlabel("Time", fontsize=15)    
ax.collections[0].colorbar.set_label("Water Temperature  ($^\circ$C)")
xticks_ix = np.array(ax.get_xticks()).astype(int)
time_label = times_validation[xticks_ix]
#nelement = len(times_validation)//N_pts
#time_label = time_label[::nelement]
#ax.xaxis.set_major_locator(plt.MaxNLocator(N_pts))
...
times_validation = times_validation.date

time_label = np.array([time[:10] for time in times_validation])
time_label = time_label[::depth_steps]
times = time_label.date
...
sns.heatmap(temp_validation, ax=ax, cmap='Spectral_r', vmin=0., vmax=35.)
ax.set_ylabel("Depth (m)", fontsize=15)
...
print(xticks_ix)
#
time_label = times_validation[xticks_ix[xticks_ix < len(times_validation)]]
